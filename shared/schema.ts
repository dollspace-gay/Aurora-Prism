...
export const posts = pgTable(
  'posts',
  {
    ...
    violatesThreadGate: boolean('violates_thread_gate')
      .default(false)
      .notNull(),
    violatesEmbeddingRules: boolean('violates_embedding_rules')
      .default(false)
      .notNull(),
...
export const postAggregations = pgTable(
  'post_aggregations',
  {
  ...
  (table) => ({
    ...
    repostCountIdx: index('idx_post_aggregations_repost_count').on(
      table.repostCount
    ),
    replyCountIdx: index('idx_post_aggregations_reply_count').on(
      table.replyCount
    ),
    bookmarkCountIdx: index('idx_post_aggregations_bookmark_count').on(
      table.bookmarkCount
    ),
    quoteCountIdx: index('idx_post_aggregations_quote_count').on(
      table.quoteCount
    ),
  })
);
...
export const postViewerStates = pgTable(
  'post_viewer_states',
  {
  ...
  (table) => ({
    postViewerIdx: uniqueIndex('idx_post_viewer_states_post_viewer').on(
      table.postUri,
      table.viewerDid
    ),
...
export const threadContexts = pgTable(
  ...
  (table) => ({
    rootAuthorLikeIdx: index('idx_thread_contexts_root_author_like').on(
      table.rootAuthorLikeUri
    ),
  })
);
...
export const likes = pgTable(
  ...
  (table) => ({
    ...
    uniqueLike: uniqueIndex('unique_like_user_post').on(
      table.userDid,
      table.postUri
    ),
  })
...
export const reposts = pgTable(
  ...
  (table) => ({
    ...
    uniqueRepost: uniqueIndex('unique_repost_user_post').on(
      table.userDid,
      table.postUri
    ),
  })
...
export const bookmarks = pgTable(
  ...
  (table) => ({
    ...
    uniqueBookmark: uniqueIndex('unique_bookmark_user_post').on(
      table.userDid,
      table.postUri
    ),
  })
...
export const quotes = pgTable(
  ...
  (table) => ({
    ...
    uniqueQuote: uniqueIndex('unique_quote_post_quoted').on(
      table.postUri,
      table.quotedUri
    ),
  })
...
export const follows = pgTable(
  ...
  (table) => ({
    ...
    uniqueFollow: uniqueIndex('unique_follow_follower_following').on(
      table.followerDid,
      table.followingDid
    ),
  })
...
export const blocks = pgTable(
  ...
  (table) => ({
    ...
    uniqueBlock: uniqueIndex('unique_block_blocker_blocked').on(
      table.blockerDid,
      table.blockedDid
    ),
  })
...
export const mutes = pgTable(
  ...
  (table) => ({
    ...
    uniqueMute: uniqueIndex('unique_mute_muter_muted').on(
      table.muterDid,
      table.mutedDid
    ),
  })
...
export const listMutes = pgTable(
  'list_mutes',
  {
    uri: varchar('uri', { length: 512 }).primaryKey(),
    muterDid: varchar('muter_did', { length: 255 }).notNull(), // No FK - can reference external users
  ...
  (table) => ({
    ...
    uniqueListMute: uniqueIndex('unique_list_mute').on(
      table.muterDid,
      table.listUri
    ),
  })
...
export const listBlocks = pgTable(
  'list_blocks',
  {
    uri: varchar('uri', { length: 512 }).primaryKey(),
    blockerDid: varchar('blocker_did', { length: 255 }).notNull(), // No FK - can reference external users
  ...
  (table) => ({
    ...
    uniqueListBlock: uniqueIndex('unique_list_block').on(
      table.blockerDid,
      table.listUri
    ),
  })
...
export const threadMutes = pgTable(
  ...
  (table) => ({
    ...
    uniqueThreadMute: uniqueIndex('unique_thread_mute').on(
      table.muterDid,
      table.threadRootUri
    ),
  })
...
export const authorizedAdmins = pgTable(
  'authorized_admins',
  {
    did: varchar('did', { length: 255 }).primaryKey(),
    handle: varchar('handle', { length: 255 }).notNull(),
...
export const userSettings = pgTable('user_settings', {
  ...
  dataCollectionForbidden: boolean('data_collection_forbidden')
    .default(false)
    .notNull(),
...
export const labelEvents = pgTable(
  ...
  {
    id: serial('id').primaryKey(),
    labelUri: varchar('label_uri', { length: 512 })
      .notNull()
      .references(() => labels.uri, { onDelete: 'cascade' }),
...
export const moderationReports = pgTable(
  ...
  (table) => ({
    ...
    createdAtIdx: index('idx_moderation_reports_created_at').on(
      table.createdAt
    ),
  })
);
...
export const moderationActions = pgTable(
  ...
  (table) => ({
    ...
    moderatorIdx: index('idx_moderation_actions_moderator').on(
      table.moderatorDid
    ),
    createdAtIdx: index('idx_moderation_actions_created_at').on(
      table.createdAt
    ),
  })
);
...
export const moderatorAssignments = pgTable(
  ...
  (table) => ({
    ...
    moderatorIdx: index('idx_moderator_assignments_moderator').on(
      table.moderatorDid
    ),
    uniqueAssignment: uniqueIndex('unique_assignment_report_moderator').on(
      table.reportId,
      table.moderatorDid
    ),
  })
...
export const activitySubscriptions = pgTable(
  ...
  (table) => ({
    subscriberIdx: index('idx_activity_subscriptions_subscriber').on(
      table.subscriberDid
    ),
    subjectIdx: index('idx_activity_subscriptions_subject').on(
      table.subjectDid
    ),
    uniqueSubscription: uniqueIndex('unique_activity_subscription').on(
      table.subscriberDid,
      table.subjectDid
    ),
  })
...
export const listItems = pgTable(
  ...
  (table) => ({
    ...
    uniqueListItem: uniqueIndex('unique_list_item').on(
      table.listUri,
      table.subjectDid
    ),
  })
...
export const starterPacks = pgTable(
  'starter_packs',
  {
    ...
    feeds: jsonb('feeds')
      .default(sql`'[]'::jsonb`)
      .notNull(), // Array of feed generator URIs
    createdAt: timestamp('created_at').notNull(),
    indexedAt: timestamp('indexed_at').defaultNow().notNull(),
  },
  (table) => ({
...
export const verifications = pgTable(
  ...
  (table) => ({
    ...
    uniqueVerification: uniqueIndex('unique_verification_subject').on(
      table.subjectDid
    ),
  })
);
...
export const firehoseCursor = pgTable(
  'firehose_cursor',
  {
    id: serial('id').primaryKey(),
...
export const genericRecords = pgTable(
  'generic_records',
  {
    uri: varchar('uri', { length: 512 }).primaryKey(),
    cid: varchar('cid', { length: 255 }).notNull(),
...
export const threadGates = pgTable(
  'thread_gates',
  {
    ...
    allowListUris: jsonb('allow_list_uris')
      .$type<string[]>()
      .default([])
      .notNull(),
    createdAt: timestamp('created_at').defaultNow().notNull(),
...
export const feedItemsRelations = relations(feedItems, ({ one }) => ({
  ...
  originator: one(users, {
    fields: [feedItems.originatorDid],
...
export const followsRelations = relations(follows, ({ one }) => ({
  follower: one(users, {
    fields: [follows.followerDid],
    references: [users.did],
    relationName: 'follower',
  }),
  following: one(users, {
    fields: [follows.followingDid],
    references: [users.did],
...
export const blocksRelations = relations(blocks, ({ one }) => ({
  blocker: one(users, {
    fields: [blocks.blockerDid],
    references: [users.did],
    relationName: 'blocker',
  }),
  blocked: one(users, {
    fields: [blocks.blockedDid],
    references: [users.did],
...
export const mutesRelations = relations(mutes, ({ one }) => ({
  muter: one(users, {
    fields: [mutes.muterDid],
    references: [users.did],
    relationName: 'muter',
  }),
  muted: one(users, {
    fields: [mutes.mutedDid],
    references: [users.did],
    relationName: 'muted',
  }),
}));
...
export const userPreferencesRelations = relations(
  ...
  ({ one }) => ({
    user: one(users, {
      fields: [userPreferences.userDid],
      references: [users.did],
    }),
  })
);
...
export const labelEventsRelations = relations(labelEvents, ({ one }) => ({
  label: one(labels, {
    fields: [labelEvents.labelUri],
    references: [labels.uri],
  }),
}));
...
export const moderatorAssignmentsRelations = relations(
  ...
  ({ one }) => ({
    report: one(moderationReports, {
      fields: [moderatorAssignments.reportId],
      references: [moderationReports.id],
    }),
  })
);
...
export const notificationsRelations = relations(notifications, ({ one }) => ({
  recipient: one(users, {
    fields: [notifications.recipientDid],
    references: [users.did],
    relationName: 'notificationsReceived',
  }),
  author: one(users, {
    fields: [notifications.authorDid],
    references: [users.did],
...
export const feedGeneratorsRelations = relations(feedGenerators, ({ one }) => ({
  creator: one(users, {
    fields: [feedGenerators.creatorDid],
    references: [users.did],
  }),
}));
...
export const starterPacksRelations = relations(starterPacks, ({ one }) => ({
  creator: one(users, {
    fields: [starterPacks.creatorDid],
    references: [users.did],
  }),
  list: one(lists, { fields: [starterPacks.listUri], references: [lists.uri] }),
}));

export const labelerServicesRelations = relations(
  labelerServices,
  ({ one }) => ({
    creator: one(users, {
      fields: [labelerServices.creatorDid],
      references: [users.did],
    }),
  })
);
...
export const insertListMuteSchema = createInsertSchema(listMutes).omit({
  indexedAt: true,
});
export const insertListBlockSchema = createInsertSchema(listBlocks).omit({
  indexedAt: true,
});
export const insertThreadMuteSchema = createInsertSchema(threadMutes).omit({
  indexedAt: true,
});
...
export const insertAuthorizedAdminSchema = createInsertSchema(
  authorizedAdmins
).omit({ addedAt: true });
...
export const insertLabelDefinitionSchema = createInsertSchema(
  labelDefinitions
).omit({ id: true, createdAt: true, updatedAt: true });
export const insertLabelEventSchema = createInsertSchema(labelEvents).omit({
  id: true,
  createdAt: true,
});
...
export const insertModeratorAssignmentSchema = createInsertSchema(
  moderatorAssignments
...
export const insertFeedGeneratorSchema = createInsertSchema(
  feedGenerators
).omit({ indexedAt: true, likeCount: true });
export const insertStarterPackSchema = createInsertSchema(starterPacks).omit({
  indexedAt: true,
});
export const insertLabelerServiceSchema = createInsertSchema(
  labelerServices
).omit({ indexedAt: true, likeCount: true });
...
export const insertActivitySubscriptionSchema = createInsertSchema(
  activitySubscriptions
).omit({ indexedAt: true });
export const insertFirehoseCursorSchema = createInsertSchema(
  firehoseCursor
).omit({ id: true, updatedAt: true });
export const insertGenericRecordSchema = createInsertSchema(
  genericRecords
).omit({ indexedAt: true });
...
export type InsertModeratorAssignment = z.infer<
  typeof insertModeratorAssignmentSchema
>;
...
export type InsertActivitySubscription = z.infer<
  typeof insertActivitySubscriptionSchema
>;
...
export type PostAggregation = typeof postAggregations.$inferSelect;
export type InsertPostAggregation = typeof postAggregations.$inferInsert;
export type PostViewerState = typeof postViewerStates.$inferSelect;
export type InsertPostViewerState = typeof postViewerStates.$inferInsert;
export type ThreadContext = typeof threadContexts.$inferSelect;
export type InsertThreadContext = typeof threadContexts.$inferInsert;
