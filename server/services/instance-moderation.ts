...
class InstanceModerationService {
  ...
  async applyInstanceLabel(params: {
    ...
    if (!INSTANCE_CONFIG.enabled) {
      console.log(
        '[INSTANCE_MOD] Instance moderation disabled, skipping label'
      );
      return;
    }
    ...
    console.log(
      `[INSTANCE_MOD] Applied label '${params.labelValue}' to ${params.subject}`,
      {
  ...
  private async deleteContentReference(subject: string): Promise<void> {
    try {
      ...
      console.error(
        `[INSTANCE_MOD] Error deleting reference for ${subject}:`,
        error
      );
    }
  }
  ...
  async getStatistics() {
    try {
      ...
      const recentTakedowns = labels.filter(
        (label) =>
          [
            'dmca-takedown',
            'court-order',
            'dsa-removal',
            'illegal-content',
          ].includes(label.val) && new Date(label.createdAt) >= thirtyDaysAgo
      );
