...
export class DIDResolver {
  ...
  configure(options: {
    ...
    if (options.baseTimeout !== undefined)
    ...
    if (options.circuitBreakerTimeout !== undefined)
      this.circuitBreakerTimeout = options.circuitBreakerTimeout;
    if (options.maxConcurrentRequests !== undefined)
      this.requestQueue.setMaxConcurrent(options.maxConcurrentRequests);
    ...
    if (options.cacheSize !== undefined || options.cacheTTL !== undefined) {
      ...
      smartConsole.log(
        `[DID_RESOLVER] Cache recreated with size: ${size}, TTL: ${ttl}ms`
      );
    }
  }
  ...
  private isCircuitOpen(): boolean {
    ...
    if (Date.now() - this.lastFailureTime > this.circuitBreakerTimeout) {
      ...
      this.failureCount = 0;
      smartConsole.log(
        '[DID_RESOLVER] Circuit breaker reset, attempting resolution again'
      );
      return false;
    }
  ...
  private recordFailure(): void {
    ...
    if (this.failureCount >= this.circuitBreakerThreshold) {
      ...
      smartConsole.warn(
        `[DID_RESOLVER] Circuit breaker opened after ${this.failureCount} consecutive failures`
      );
    }
  }
      ...
      try {
        ...
        if (error instanceof Error) {
          if (error.name === 'TimeoutError') {
            smartConsole.warn(
              `[DID_RESOLVER] Timeout on attempt ${attempt + 1}, retrying in ${delay}ms`
            );
          } else if (error.message.includes('fetch')) {
            smartConsole.warn(
              `[DID_RESOLVER] Network error on attempt ${attempt + 1}, retrying in ${delay}ms:`,
              error.message
            );
          } else {
            smartConsole.warn(
              `[DID_RESOLVER] Attempt ${attempt + 1} failed, retrying in ${delay}ms:`,
              error.message
            );
          }
        } else {
  ...
  async resolveHandle(handle: string): Promise<string | null> {
    try {
      ...
      smartConsole.error(
        `[DID_RESOLVER] Error resolving handle ${handle}:`,
        error
      );
      return null;
    }
    ...
    try {
      ...
      for (const record of txtRecords) {
        ...
        if (did.startsWith('did:')) {
          ...
          if (!did.startsWith('did:plc:') && !did.startsWith('did:web:')) {
            smartConsole.warn(
              `[DID_RESOLVER] Unsupported DID method in DNS for ${handle}: ${did}`
            );
          }
          return did;
        }
      ...
      if (error instanceof Error) {
        ...
        if (
          ...
          smartConsole.warn(
            `[DID_RESOLVER] DNS error for ${handle}:`,
            error.message
          );
        }
      }
  ...
  private async resolveHandleViaHTTPS(handle: string): Promise<string | null> {
    try {
      ...
      if (!response.ok) {
        ...
        smartConsole.warn(
          `[DID_RESOLVER] HTTP ${response.status} for ${handle}/.well-known/atproto-did`
        );
        return null;
      }
      ...
      if (
        ...
        if (Math.random() < 0.01) {
          // Log 1% of occurrences to avoid spam
          smartConsole.warn(
            `[DID_RESOLVER] Domain ${handle} returns ${contentType} instead of AT Protocol DID (likely has website but no AT Protocol config)`
          );
        }
        return null;
      }
      ...
      if (!did.startsWith('did:plc:') && !did.startsWith('did:web:')) {
        smartConsole.warn(
          `[DID_RESOLVER] Unsupported DID method for ${handle}: ${did}`
        );
      ...
      if (error instanceof Error) {
        if (error.name === 'TimeoutError' || error.name === 'AbortError') {
          smartConsole.warn(
            `[DID_RESOLVER] Timeout resolving ${handle}/.well-known/atproto-did`
          );
        } else if (
          ...
          smartConsole.warn(
            `[DID_RESOLVER] Network error for ${handle}:`,
            error.message
          );
        }
  ...
  private async resolvePLCDID(did: string): Promise<DIDDocument | null> {
    ...
    if (this.isCircuitOpen()) {
      smartConsole.warn(
        `[DID_RESOLVER] Circuit breaker is open, skipping PLC DID resolution for ${did}`
      );
      return null;
    }
    ...
    try {
      ...
      const result = await this.requestQueue.enqueue(async () => {
        return await this.retryWithBackoff(async () => {
          ...
          if (!response.ok) {
            if (response.status === 404) {
              smartConsole.warn(
                `[DID_RESOLVER] DID not found in PLC directory: ${did}`
              );
              return null;
            }
            if (response.status >= 500) {
              throw new Error(
                `PLC directory server error ${response.status}: ${response.statusText}`
              );
            }
          ...
          if (
            ...
            !contentType.includes('application/did+ld+json')
          ) {
            smartConsole.warn(
              `[DID_RESOLVER] Unexpected content type from PLC directory for ${did}: ${contentType}`
            );
          }
          ...
          if (!data || typeof data !== 'object') {
            smartConsole.warn(
              `[DID_RESOLVER] Invalid DID document format from PLC for ${did}: not an object`
            );
          ...
          if (!data.id) {
            smartConsole.warn(
              `[DID_RESOLVER] Invalid DID document format from PLC for ${did}: missing id field`
            );
          ...
          if (data.id !== did) {
            smartConsole.error(
              `[DID_RESOLVER] SECURITY: DID mismatch from PLC for ${did}: document contains ${data.id}`
            );
            throw new Error(
              `DID document ID mismatch: expected ${did}, got ${data.id}. Possible impersonation attempt.`
            );
          }
      ...
      if (error instanceof Error) {
        if (error.name === 'TimeoutError' || error.name === 'AbortError') {
          smartConsole.error(
            `[DID_RESOLVER] Timeout resolving PLC DID ${did} after ${this.maxRetries + 1} attempts`
          );
        } else if (
          error.message.includes('fetch') ||
          error.message.includes('network')
          ...
          smartConsole.error(
            `[DID_RESOLVER] Network error resolving PLC DID ${did}:`,
            error.message
          );
        } else {
          smartConsole.error(
            `[DID_RESOLVER] Error resolving PLC DID ${did}:`,
            error.message
          );
        }
      } else {
        smartConsole.error(
          `[DID_RESOLVER] Unknown error resolving PLC DID ${did}:`,
          error
        );
      }
      return null;
  ...
  private async resolveWebDID(did: string): Promise<DIDDocument | null> {
    try {
      return await this.retryWithBackoff(async () => {
        ...
        const path =
          didParts.length > 1 ? '/' + didParts.slice(1).join('/') : '';
        ...
        if (!response.ok) {
          if (response.status === 404) {
            smartConsole.warn(
              `[DID_RESOLVER] Web DID not found: ${did} at ${didDocUrl}`
            );
            return null;
          }
          throw new Error(`HTTP ${response.status}: ${response.statusText}`);
        ...
        if (
          ...
          smartConsole.warn(
            `[DID_RESOLVER] Unexpected content type for Web DID ${did}: ${contentType}`
          );
        }
        ...
        if (!data || typeof data !== 'object') {
          smartConsole.warn(
            `[DID_RESOLVER] Invalid DID document format for ${did}: not an object`
          );
          throw new Error('Invalid DID document format: not an object');
        }
        ...
        if (!data.id) {
          smartConsole.warn(
            `[DID_RESOLVER] Invalid DID document format for ${did}: missing id field`
          );
          throw new Error('Invalid DID document format: missing id');
        }
        ...
        if (data.id !== did) {
          smartConsole.error(
            `[DID_RESOLVER] SECURITY: DID mismatch for did:web ${did}: document contains ${data.id}`
          );
          throw new Error(
            `DID document ID mismatch: expected ${did}, got ${data.id}. Possible impersonation attempt.`
          );
      ...
      if (error instanceof Error) {
        if (error.name === 'TimeoutError') {
          smartConsole.error(
            `[DID_RESOLVER] Timeout resolving Web DID ${did} after ${this.maxRetries + 1} attempts`
          );
        } else if (error.message.includes('fetch')) {
          smartConsole.error(
            `[DID_RESOLVER] Network error resolving Web DID ${did}:`,
            error.message
          );
        } else {
          smartConsole.error(
            `[DID_RESOLVER] Error resolving Web DID ${did}:`,
            error.message
          );
        }
      } else {
        smartConsole.error(
          `[DID_RESOLVER] Unknown error resolving Web DID ${did}:`,
          error
        );
      }
      return null;
  ...
  getPDSEndpoint(didDoc: DIDDocument): string | null {
    if (!didDoc.service || !Array.isArray(didDoc.service)) {
      smartConsole.warn(
        `[DID_RESOLVER] No services array found in DID document for ${didDoc.id}`
      );
      return null;
    }
    ...
    if (!pdsService) {
      smartConsole.warn(
        `[DID_RESOLVER] No PDS service found in DID document for ${didDoc.id}`
      );
      smartConsole.warn(
        `[DID_RESOLVER] Available services: ${didDoc.service.map((s) => `${s.id}:${s.type}`).join(', ')}`
      );
      return null;
    }
    ...
    if (!endpoint || typeof endpoint !== 'string') {
      smartConsole.warn(
        `[DID_RESOLVER] Invalid PDS endpoint format for ${didDoc.id}: ${endpoint}`
      );
      return null;
    }
    ...
    if (!endpoint.startsWith('https://') && !endpoint.startsWith('http://')) {
      smartConsole.warn(
        `[DID_RESOLVER] PDS endpoint must be HTTP(S) URL for ${didDoc.id}: ${endpoint}`
      );
      return null;
    }
    ...
    if (!isUrlSafeToFetch(endpoint)) {
      smartConsole.warn(
        `[DID_RESOLVER] PDS endpoint failed security validation for ${didDoc.id}: ${endpoint}`
      );
      return null;
    }
  ...
  async resolveHandleToPDS(handle: string): Promise<{
    ...
    try {
      ...
      if (!did) {
        smartConsole.error(
          `[DID_RESOLVER] Could not resolve handle ${handle} to DID`
        );
        return null;
      }
      ...
      if (!didDoc) {
        smartConsole.error(
          `[DID_RESOLVER] Could not resolve DID ${did} to document`
        );
        return null;
      }
      ...
      if (!this.verifyHandle(didDoc, handle)) {
        smartConsole.error(
          `[DID_RESOLVER] Handle ${handle} not confirmed in DID document`
        );
        return null;
      }
      ...
      if (!pdsEndpoint) {
        smartConsole.error(
          `[DID_RESOLVER] No PDS endpoint found in DID document for ${did}`
        );
        return null;
      }
      ...
      smartConsole.error(
        `[DID_RESOLVER] Error resolving handle ${handle} to PDS:`,
        error
      );
      return null;
    }
  ...
  async resolveDIDToPDS(did: string): Promise<string | null> {
    try {
      ...
      smartConsole.error(
        `[DID_RESOLVER] Error resolving DID ${did} to PDS:`,
        error
      );
      return null;
    }
  ...
  getFeedGeneratorEndpoint(didDoc: DIDDocument): string | null {
    ...
    if (!endpoint || typeof endpoint !== 'string') {
      smartConsole.warn(
        `[DID_RESOLVER] Invalid Feed Generator endpoint format for ${didDoc.id}: ${endpoint}`
      );
      return null;
    }
    ...
    if (!endpoint.startsWith('https://') && !endpoint.startsWith('http://')) {
      smartConsole.warn(
        `[DID_RESOLVER] Feed Generator endpoint must be HTTP(S) URL for ${didDoc.id}: ${endpoint}`
      );
      return null;
    }
    ...
    if (!isUrlSafeToFetch(endpoint)) {
      smartConsole.warn(
        `[DID_RESOLVER] Feed Generator endpoint failed security validation for ${didDoc.id}: ${endpoint}`
      );
      return null;
    }
  ...
  async resolveDIDToFeedGenerator(did: string): Promise<string | null> {
    try {
      ...
      smartConsole.error(
        `[DID_RESOLVER] Error resolving DID ${did} to Feed Generator:`,
        error
      );
      return null;
    }
  ...
  getHandleFromDIDDocument(didDoc: DIDDocument): string | null {
    if (
      ...
      didDoc.alsoKnownAs.length === 0
    ) {
      return null;
    }

    // Find handle URI in alsoKnownAs field (format: at://username.domain)
    const handleUri = didDoc.alsoKnownAs.find(
      (uri) => typeof uri === 'string' && uri.startsWith('at://')
    );
    ...
    if (handleUri) {
      ...
      smartConsole.warn(
        `[DID_RESOLVER] Invalid handle format in alsoKnownAs for ${didDoc.id}: ${handle}`
      );
    }

    // Log available alsoKnownAs values for debugging
    smartConsole.warn(
      `[DID_RESOLVER] No valid handle found in alsoKnownAs for ${didDoc.id}:`,
      didDoc.alsoKnownAs
    );
    return null;
  }
  ...
  async resolveDIDToHandle(did: string): Promise<string | null> {
    ...
    try {
      ...
      if (!didDoc) {
        smartConsole.warn(
          `[DID_RESOLVER] Could not resolve DID document for ${did}`
        );
        return null;
      }
      ...
      if (!handle) {
        smartConsole.warn(
          `[DID_RESOLVER] No handle found in DID document for ${did}`
        );
        return null;
      }
      ...
      if (this.resolutionCount % this.BATCH_LOG_SIZE === 0) {
        ...
        smartConsole.log(
          `[DID_RESOLVER] Resolved ${this.BATCH_LOG_SIZE} DIDs (total: ${this.resolutionCount}, cache hit rate: ${cacheHitRate.toFixed(1)}%)`
        );
      }
      ...
      smartConsole.error(
        `[DID_RESOLVER] Error resolving DID ${did} to handle:`,
        error
      );
      return null;
    }
