...
export async function serializePosts(
  ...
  if (authors.length !== authorDids.length) {
    ...
    console.warn(
      `[SERIALIZE_POSTS] ${missingDids.length} authors missing from database:`,
      missingDids
    );
  }
  ...
  return posts
    .map((post) => {
      ...
      if (
        ...
        console.warn(
          `[SERIALIZE_POSTS] Author ${post.authorDid} has invalid handle (got: ${authorHandle || 'undefined'}), using fallback: ${INVALID_HANDLE}`
        );
        ...
        author: (() => {
          ...
          if (author?.avatarUrl) {
            if (author.avatarUrl.startsWith('http')) {
              ...
              avatarUrl = transformBlobToCdnUrl(
                author.avatarUrl,
                author.did,
                'avatar',
                req
          ...
          return {
            ...
            viewer: createAuthorViewerState(
              post.authorDid,
              listMutes,
              listBlocks,
              listData
