...
export class HydrationDataLoader {
  ...
  async loadPosts(postUris: string[], viewerDid?: string) {
    ...
    const [authors, aggregations, viewerStates, labels] = await Promise.all([
      ...
      viewerDid
        ? Promise.all(
            validPosts.map((p) =>
              this.viewerStateLoader.load(`${p.uri}:${viewerDid}`)
            )
          )
        : [],
    ...
    const aggregationMap = new Map(
      validPosts.map((p, i) => [p.uri, aggregations[i]])
    );
    const viewerStateMap = new Map(
      validPosts.map((p, i) => [p.uri, viewerStates[i]])
    );
  ...
  private createViewerStateLoader() {
    return new DataLoader<string, any>(
      async (keys) => {
        ...
        const postUris = [...new Set(validKeys.map((k) => k.postUri))].filter(
          Boolean
        );
        const viewerDids = [
          ...new Set(validKeys.map((k) => k.viewerDid)),
        ].filter(Boolean);
        ...
        const [likesResult, repostsResult, bookmarksResult, statesResult] =
          await Promise.all([
        ...
        const likeMap = new Map(
          likesResult.map((l) => [`${l.postUri}:${l.userDid}`, true])
        );
        const repostMap = new Map(
          repostsResult.map((r) => [`${r.postUri}:${r.userDid}`, r.uri])
        );
        const bookmarkMap = new Map(
          bookmarksResult.map((b) => [`${b.postUri}:${b.userDid}`, true])
        );
        const stateMap = new Map(
          statesResult.map((s) => [`${s.postUri}:${s.viewerDid}`, s])
        );
  ...
  private createActorViewerStateLoader() {
    return new DataLoader<string, any>(
      async (keys) => {
        ...
        // Get unique values for efficient querying
        const actorDids = [...new Set(validKeys.map((k) => k.actorDid))].filter(
          Boolean
        );
        const viewerDids = [
          ...new Set(validKeys.map((k) => k.viewerDid)),
        ...
        const [followingResult, followedByResult, blocksResult, mutesResult] =
          await Promise.all([
        ...
        const followingMap = new Map(
          followingResult.map((f) => [
            `${f.followingDid}:${f.followerDid}`,
            f.uri,
          ])
        );
        const followedByMap = new Map(
          followedByResult.map((f) => [
            `${f.followerDid}:${f.followingDid}`,
            f.uri,
          ])
        );
        const blockingMap = new Map(
          blocksResult
            .filter(
              (b) =>
                actorDids.includes(b.blockedDid) &&
                viewerDids.includes(b.blockerDid)
            )
        ...
        const blockedByMap = new Map(
          blocksResult
            .filter(
              (b) =>
                actorDids.includes(b.blockerDid) &&
                viewerDids.includes(b.blockedDid)
            )
            .map((b) => [`${b.blockerDid}:${b.blockedDid}`, true])
        );
        const muteMap = new Map(
          mutesResult.map((m) => [`${m.mutedDid}:${m.muterDid}`, true])
        );
  ...
  get aggregations() {
    return this.aggregationLoader;
  }
  get viewerStates() {
    return this.viewerStateLoader;
  }
  get actorViewerStates() {
    return this.actorViewerStateLoader;
  }
  get threadGates() {
    return this.threadGateLoader;
  }
  get postGates() {
    return this.postGateLoader;
  }
  get labels() {
    return this.labelLoader;
  }
}
