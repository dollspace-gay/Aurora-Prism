...
export class ViewerContextBuilder {
  ...
  async build(viewerDid: string): Promise<ViewerContext> {
    const [
      ...
      // Blocking
      db
        .select({ blockedDid: blocks.blockedDid })
        .from(blocks)
      ...
      // Muting
      db
        .select({ mutedDid: mutes.mutedDid })
        .from(mutes)
        .where(eq(mutes.muterDid, viewerDid)),

      // Thread mutes
      db
        .select({ threadRootUri: threadMutes.threadRootUri })
        .from(threadMutes)
    ...
    if (listMutesData.length > 0) {
      ...
      const listMembersData = await db
        .select({
          listUri: sql<string>`list_uri`,
          memberDid: sql<string>`member_did`,
        })
  ...
  async buildActorStates(
    ...
    for (const did of actorDids) {
      ...
      // Followed by
      const followedByUri = followedByUris.find(
        (f) => f.followerDid === did
      )?.uri;
      if (followedByUri) {
  ...
  async buildPostStates(
    ...
    const [likesData, repostsData, bookmarksData] = await Promise.all([
      db
        .select({ postUri: sql<string>`post_uri`, uri: sql<string>`uri` })
        .from(sql`likes`)
        .where(
          and(eq(sql`user_did`, viewerDid), inArray(sql`post_uri`, postUris))
        ),
      ...
      db
        .select({ postUri: sql<string>`post_uri` })
        .from(sql`bookmarks`)
        .where(
          and(eq(sql`user_did`, viewerDid), inArray(sql`post_uri`, postUris))
        ),
