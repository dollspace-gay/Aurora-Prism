...
function sanitizeCSSValue(value: string | undefined): string {
  ...
  // HSL/HSLA
  if (
    /^hsla?\(\s*\d+\s*,\s*\d+%\s*,\s*\d+%(\s*,\s*[\d.]+)?\s*\)$/.test(trimmed)
  ) {
    return trimmed;
  }
  ...
  const namedColors =
    /^(transparent|currentColor|inherit|black|white|red|green|blue|yellow|orange|purple|pink|gray|grey)$/i;
  ...
  console.warn(
    '[Chart] Invalid CSS color value detected, using transparent:',
    value
  );
  return 'transparent';
}
                          ...
                          style={
                            {
                              ...
                              '--color-border':
                                sanitizeCSSValue(indicatorColor),
